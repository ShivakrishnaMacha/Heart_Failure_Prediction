...
# === Set Page Configuration (Move this to the very top) ===
...
if risk_prediction is not None:
    with st.expander("📤 Export and Share Your Results"):
        if st.button("📄 Download My Report (PDF)"):
            ...
            pdf.cell(200, 10, txt=f"Heart Risk Status: {'High Risk' if risk_prediction==1 else 'Low Risk'}", ln=True)
            pdf.ln(10)
            pdf.multi_cell(0, 10, txt="Thank you for using Heart Health Risk Analyzer! Stay healthy! 💖")
            
            pdf_output_path = os.path.join(base_path, "heart_risk_report.pdf")
